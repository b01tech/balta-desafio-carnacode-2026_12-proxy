...
public class DocumentRepository : IDocumentRepository
{
    ...
    public ConfidentialDocument? GetDocument(string documentId)
    {
        ...
        Thread.Sleep(500);
        if (_database.TryGetValue(documentId, out var doc))
        {
            Console.WriteLine(
                $"[Repository] Documento carregado: {doc.SizeInBytes / (1024 * 1024)} MB"
            );
            return doc;
        }
        return null;
    ...
    public void UpdateDocument(string documentId, string newContent)
    {
        ...
        if (_database.ContainsKey(documentId))
        {
            ...
            _database[documentId].SizeInBytes = newContent.Length * 2L;
        }
    }
}
